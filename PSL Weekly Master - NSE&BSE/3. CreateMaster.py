...
codes = natsorted(pd.concat([pd.read_csv(p, usecols=['code', 'from_dte', 'to_dte']) for p in glob('parameters/*.csv')])['code'].dropna().unique())

indices = sorted(pd.concat([pd.read_csv(p, usecols=['index', 'code', 'from_dte', 'to_dte']) for p in glob('parameters/*.csv')])['index'].dropna().unique())
iCount = len(indices)
...
for index in indices:
    ...
    for code in codes:
        ...
        if sl_time_csvs:
            dfs_to_concat = []
            for f in sl_time_csvs:
                df = pd.read_csv(f)
                df['Start.Date'] = pd.to_datetime(df['Start.Date'])
                df = df.set_index('Start.Date')
                df.drop(columns=['Start.DTE','End.DTE','End.Date'], inplace=True)
                dfs_to_concat.append(df)
            
            df = pd.concat(dfs_to_concat, axis=0)
            df = df.groupby(level=0).sum()  # Handle duplicate indices by summing
            df = df.reindex(pd.to_datetime(master_df['Date']), fill_value=0)
            sl_times_dfs.append(df)
