...
def get_parameter_data(code, parameter_path):
    ...
    if code.startswith("NRE_CC") and code.endswith("PSL"):
...
def NRE_CC_per_minute_mtm(bt, start_time, end_time, orderside, method, sl, om, re_entries, seperate=False):
    try:
...
def NRE_CC_PSL(bt, start_time, end_time, last_trade_time, trade_interval, orderside, method, sl, om, re_entries):
    try:
        start_dt = datetime.datetime.combine(bt.current_date, start_time)
        end_dt = datetime.datetime.combine(bt.current_date, end_time)
        ...
        per_minute_trades = [NRE_CC_per_minute_mtm(bt, re_time, end_time, orderside, method, sl, om, re_entries) for re_time in time_range]
                ...
                for current_date in date_lists:
                    ...
                    if not is_file_exists(output_csv_path, file_name, parameter_len):
                        ...
                        for idx, i in enumerate(range(0, parameter_len, chunk_size), start=1):
                            ...
                            chunk = [NRE_CC_PSL(bt, row.entry_time, row.exit_time, row.last_trade_time, row.trade_interval, row.orderside, row.method, row.sl, row.om, row.re_entries) for row in tqdm(chunk_parameter.itertuples(), total=len(chunk_parameter), colour='GREEN')]
