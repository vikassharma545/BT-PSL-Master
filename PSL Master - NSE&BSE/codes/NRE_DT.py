...
def get_parameter_data(code, parameter_path):
    ...
    if code.startswith("NRE_DT") and code.endswith("PSL"):
...
def NRE_per_minute_mtm(bt, start_time, end_time, orderside, method, sl, ratio, om, re_entries, seperate=False):
    try:
        ...
        _, _, _, _, ce_sl_price, ce_sl_time, ce_mtm_data0 = bt.sl_check_single_leg_with_sl_trail(start_dt, end_dt, ce_scrip, sl=sl, trail_type='RATIO', trail_gap=ratio, orderside=orderside, from_candle_close=from_candle_close, with_ohlc=True, per_minute_mtm=True)
        _, _, _, _, pe_sl_price, pe_sl_time, pe_mtm_data0 = bt.sl_check_single_leg_with_sl_trail(start_dt, end_dt, pe_scrip, sl=sl, trail_type='RATIO', trail_gap=ratio, orderside=orderside, from_candle_close=from_candle_close, with_ohlc=True, per_minute_mtm=True)
        ce_entry_price, pe_entry_price = ce_price, pe_price
        ...
        for re_no in range(max_re):
            if ce_sl_time and re_no < re_entries:
                ...
                ce_scrip, ce_price, future_price, start_dt = bt.get_strike(start_dt, end_dt, target=ce_sl_price, obove_target_only=True, only='CE')
                _, ce_decay_flag, ce_decay_time = bt.decay_check_single_leg(start_dt, end_dt, ce_scrip, decay_price=ce_entry_price, from_candle_close=from_candle_close, orderside=orderside)

                if ce_decay_flag:
                    ce_sl_time, ce_mtm_data = bt.sl_check_single_leg_with_sl_trail(ce_decay_time, end_dt, ce_scrip, o=(None if method == 'CC' else ce_entry_price), sl=sl, trail_type='RATIO', trail_gap=ratio, orderside=orderside, from_candle_close=from_candle_close, per_minute_mtm=True)
        ...
        for re_no in range(max_re):
            if pe_sl_time and re_no < re_entries:
                ...
                pe_scrip, pe_price, future_price, start_dt = bt.get_strike(start_dt, end_dt, target=pe_sl_price, obove_target_only=True, only='PE')
                _, pe_decay_flag, pe_decay_time = bt.decay_check_single_leg(start_dt, end_dt, pe_scrip, decay_price=pe_entry_price, from_candle_close=from_candle_close, orderside=orderside)

                if pe_decay_flag:
                    pe_sl_time, pe_mtm_data = bt.sl_check_single_leg_with_sl_trail(pe_decay_time, end_dt, pe_scrip, o=(None if method == 'CC' else pe_entry_price), sl=sl, trail_type='RATIO', trail_gap=ratio, orderside=orderside, from_candle_close=from_candle_close, per_minute_mtm=True)
        ...
        print(e, [bt.index, bt.current_date, start_time, end_time, orderside, method, sl, ratio, om, re_entries])
        return

def NRE_DT_PSL(bt, start_time, end_time, last_trade_time, trade_interval, orderside, method, sl, ratio, om, re_entries):
    try:
        start_dt = datetime.datetime.combine(bt.current_date, start_time)
        end_dt = datetime.datetime.combine(bt.current_date, end_time)
        ...
        per_minute_trades = [NRE_per_minute_mtm(bt, re_time, end_time, orderside, method, sl, ratio, om, re_entries) for re_time in time_range]
        ...
        if per_minute_trades:
            ...
            return [tcode, bt.index, start_time, end_time, last_trade_time, trade_interval, orderside, method, sl, ratio, om, re_entries, bt.current_date.date(), bt.current_date.day_name(), bt.dte, entry_time.time(), minute_margin_per_share] + mtm_time_list
    except Exception as e:
        print(e, [bt.index, bt.current_date, start_time, end_time, last_trade_time, trade_interval, orderside, method, sl, ratio, om, re_entries])
        return
    ...
    for row_idx in range(len(meta_data)):
        ...
        if row_idx in meta_row_nos and meta_data.loc[row_idx, 'run']:
            ...
            try:
                ...
                log_cols = ('P_Strategy/P_Index/P_StartTime/P_EndTime/P_LastTradeTime/P_TradeInterval/P_OrderSide/P_Method/P_SL/P_Ratio/P_OM/P_ReEntries/Date/Day/DTE/Entry.Time/MMPS/')
                ...
                for current_date in date_lists:
                    ...
                    try:
                        ...
                        for idx, i in enumerate(range(0, parameter_len, chunk_size), start=1):
                            ...
                            chunk = [NRE_DT_PSL(bt, row.entry_time, row.exit_time, row.last_trade_time, row.trade_interval, row.orderside, row.method, row.sl, row.ratio, row.om, row.re_entries) for row in tqdm(chunk_parameter.itertuples(), total=len(chunk_parameter), colour='GREEN')]
