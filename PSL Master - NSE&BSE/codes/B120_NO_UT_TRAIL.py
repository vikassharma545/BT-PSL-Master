...
def get_parameter_data(code, parameter_path):
    ...
    if code.startswith("B120_NO_UT_TRAIL") and code.endswith("PSL"):
...
def b120_per_minute_mtm(bt, start_time, end_time, orderside, method, sl, om, trail, trail_upto_cost, seperate=False):
    try:
        ...
        entry_time = start_dt
        _, _, _, _, ce_sl_price, ce_sl_time, ce_mtm_data = bt.sl_check_single_leg_with_sl_trail(start_dt, end_dt, ce_scrip, sl=sl, trail_type='RATIO', trail_gap=trail, trail_upto_cost=trail_upto_cost, with_ohlc=True, orderside=orderside, from_candle_close=from_candle_close, per_minute_mtm=True)
        _, _, _, _, pe_sl_price, pe_sl_time, pe_mtm_data = bt.sl_check_single_leg_with_sl_trail(start_dt, end_dt, pe_scrip, sl=sl, trail_type='RATIO', trail_gap=trail, trail_upto_cost=trail_upto_cost, with_ohlc=True, orderside=orderside, from_candle_close=from_candle_close, per_minute_mtm=True)
        ...
        print(e, [bt.index, bt.current_date, start_time, end_time, orderside, method, sl, om, trail, trail_upto_cost])
        return

def b120_PSL(bt, start_time, end_time, last_trade_time, trade_interval, orderside, method, sl, om, trail, trail_upto_cost):
    try:
        start_dt = datetime.datetime.combine(bt.current_date, start_time)
        end_dt = datetime.datetime.combine(bt.current_date, end_time)
        ...
        per_minute_trades = [b120_per_minute_mtm(bt, re_time, end_time, orderside, method, sl, om, trail, trail_upto_cost) for re_time in time_range]
        ...
        if per_minute_trades:
            ...
            return [tcode, bt.index, start_time, end_time, last_trade_time, trade_interval, orderside, method, sl, om, trail, trail_upto_cost, bt.current_date.date(), bt.current_date.day_name(), bt.dte, entry_time.time(), minute_margin_per_share] + mtm_time_list
    except Exception as e:
        print(e, [bt.index, bt.current_date, start_time, end_time, last_trade_time, trade_interval, orderside, method, sl, om, trail, trail_upto_cost])
        return
    ...
    for row_idx in range(len(meta_data)):
        ...
        if row_idx in meta_row_nos and meta_data.loc[row_idx, 'run']:
            ...
            try:
                ...
                log_cols = ('P_Strategy/P_Index/P_StartTime/P_EndTime/P_LastTradeTime/P_TradeInterval/P_OrderSide/P_Method/P_SL/P_OM/P_Trail/P_TrailUptoCost/Date/Day/DTE/Entry.Time/MMPS/')
                ...
                for current_date in date_lists:
                    ...
                    try:
                        ...
                        for idx, i in enumerate(range(0, parameter_len, chunk_size), start=1):
                            ...
                            chunk = [b120_PSL(bt, row.entry_time, row.exit_time, row.last_trade_time, row.trade_interval, row.orderside, row.method, row.sl, row.om, row.trail, row.trail_upto_cost) for row in tqdm(chunk_parameter.itertuples(), total=len(chunk_parameter), colour='GREEN')]
