...
def get_parameter_data(code, parameter_path):
    ...
    if code.startswith("NRE_CCRESL") and code.endswith("PSL"):
...
def NRE_CC_per_minute_mtm(bt, start_time, end_time, orderside, method, sl, ce_re_sl, pe_re_sl, om, ce_re_entries, pe_re_entries, ce_till_time, pe_till_time, seperate=False):
    try:
        ...
        for re_no in range(max_re):
            if (ce_sl_time and re_no < ce_re_entries) and (ce_sl_time.time() < ce_till_time):
                start_dt = ce_sl_time
                ce_price_at_sl = bt.options_data.loc[(start_dt, ce_scrip), 'close']
                ...
                if ce_price <= ce_price_at_sl:
                    ...
                    if ce_decay_flag and (ce_decay_time.time() < ce_till_time):
                        ce_sl_time, ce_mtm_data = bt.sl_check_single_leg(ce_decay_time, end_dt, ce_scrip, o=(None if method == 'CC' else ce_price), sl=ce_re_sl, orderside=orderside, from_candle_close=from_candle_close, per_minute_mtm=True)
                    ...
                    if ce_decay_time is not None and (ce_decay_time.time() < ce_till_time):
                        ce_sl_time, ce_mtm_data = bt.sl_check_single_leg(ce_decay_time, end_dt, ce_scrip, o=(None if method == 'CC' else ce_price), sl=ce_re_sl, orderside=orderside, from_candle_close=from_candle_close, per_minute_mtm=True)
                        ce_mtm_data = set_pm_time_index(ce_mtm_data, time_index)
                        ce_re_mtm_data += ce_mtm_data
                    else:
        ...
        for re_no in range(max_re):
            if (pe_sl_time and re_no < pe_re_entries) and (pe_sl_time.time() < pe_till_time):
                start_dt = pe_sl_time
                pe_price_at_sl = bt.options_data.loc[(start_dt, pe_scrip), 'close']
                ...
                if pe_price <= pe_price_at_sl:
                    ...
                    if pe_decay_flag and (pe_decay_time.time() < pe_till_time):
                        pe_sl_time, pe_mtm_data = bt.sl_check_single_leg(pe_decay_time, end_dt, pe_scrip, o=(None if method == 'CC' else pe_price), sl=pe_re_sl, orderside=orderside, from_candle_close=from_candle_close, per_minute_mtm=True)
                    ...
                    if pe_decay_time is not None and (pe_decay_time.time() < pe_till_time):
                        pe_sl_time, pe_mtm_data = bt.sl_check_single_leg(pe_decay_time, end_dt, pe_scrip, o=(None if method == 'CC' else pe_price), sl=pe_re_sl, orderside=orderside, from_candle_close=from_candle_close, per_minute_mtm=True)
                        pe_mtm_data = set_pm_time_index(pe_mtm_data, time_index)
                        pe_re_mtm_data += pe_mtm_data
                    else:
        ...
        print(e, [bt.index, bt.current_date, start_time, end_time, orderside, method, sl, ce_re_sl, pe_re_sl, om, ce_re_entries, pe_re_entries, ce_till_time, pe_till_time])
        return

def NRE_CC_PSL(bt, start_time, end_time, last_trade_time, trade_interval, orderside, method, sl, ce_re_sl, pe_re_sl, om, ce_re_entries, pe_re_entries, ce_till_time, pe_till_time):
    try:
        start_dt = datetime.datetime.combine(bt.current_date, start_time)
        end_dt = datetime.datetime.combine(bt.current_date, end_time)
        ...
        per_minute_trades = [NRE_CC_per_minute_mtm(bt, re_time, end_time, orderside, method, sl, ce_re_sl, pe_re_sl, om, ce_re_entries, pe_re_entries, ce_till_time, pe_till_time) for re_time in time_range]
        ...
        return [tcode, bt.index, start_time, end_time, last_trade_time, trade_interval, orderside, method, sl, ce_re_sl, pe_re_sl, om, ce_re_entries, pe_re_entries, ce_till_time, pe_till_time, bt.current_date.date(), bt.current_date.day_name(), bt.dte, entry_time.time(), minute_margin_per_share] + mtm_time_list
    except Exception as e:
        print(e, [bt.index, bt.current_date, start_time, end_time, last_trade_time, trade_interval, orderside, method, sl, ce_re_sl, pe_re_sl, om, ce_re_entries, pe_re_entries, ce_till_time, pe_till_time])
        return
    ...
    for row_idx in range(len(meta_data)):
        ...
        if row_idx in meta_row_nos and meta_data.loc[row_idx, 'run']:
            ...
            try:
                ...
                log_cols = ('P_Strategy/P_Index/P_StartTime/P_EndTime/P_LastTradeTime/P_TradeInterval/P_OrderSide/P_Method/P_SL/P_CEReSL/P_PEReSL/P_OM/P_CEReEntries/P_PEReEntries/P_CETillTime/P_PETillTime/Date/Day/DTE/Entry.Time/MMPS/')
                ...
                for current_date in date_lists:
                    ...
                    try:
                        ...
                        for idx, i in enumerate(range(0, parameter_len, chunk_size), start=1):
                            ...
                            chunk = [NRE_CC_PSL(bt, row.entry_time, row.exit_time, row.last_trade_time, row.trade_interval, row.orderside, row.method, row.sl, row.ce_re_sl, row.pe_re_sl, row.om, row.ce_re_entries, row.pe_re_entries, row.ce_till_time, row.pe_till_time) for row in tqdm(chunk_parameter.itertuples(), total=len(chunk_parameter), colour='GREEN')]
