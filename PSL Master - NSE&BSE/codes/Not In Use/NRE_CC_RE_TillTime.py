...
def NRE_CC_per_minute_mtm(bt, start_time, end_time, orderside, method, sl, om, still_time, re_entries, seperate=False):
    try:
        ...
        for re_no in range(max_re):
            if (ce_sl_time and re_no < re_entries) and (ce_sl_time.time() < still_time):
                start_dt = ce_sl_time
        ...
        for re_no in range(max_re):
            if (pe_sl_time and re_no < re_entries) and (pe_sl_time.time() < still_time):
                start_dt = pe_sl_time
        ...
        print(e, [bt.index, bt.current_date, start_time, end_time, orderside, method, sl, om, still_time, re_entries])
        return

def NRE_CC_PSL(bt, start_time, end_time, last_trade_time, trade_interval, orderside, method, sl, om, still_time, re_entries):
    try:
        start_dt = datetime.datetime.combine(bt.current_date, start_time)
        end_dt = datetime.datetime.combine(bt.current_date, end_time)
        ...
        per_minute_trades = [NRE_CC_per_minute_mtm(bt, re_time, end_time, orderside, method, sl, om, still_time, re_entries) for re_time in time_range]
        ...
        if per_minute_trades:
            ...
            return [tcode, bt.index, start_time, end_time, last_trade_time, trade_interval, orderside, method, sl, om, still_time, re_entries, bt.current_date.date(), bt.current_date.day_name(), bt.dte, entry_time.time(), minute_margin_per_share] + mtm_time_list
    except Exception as e:
        print(e, [bt.index, bt.current_date, start_time, end_time, last_trade_time, trade_interval, orderside, method, sl, om, still_time, re_entries])
        return
    ...
    for row_idx in range(len(meta_data)):
        ...
        if row_idx in meta_row_nos and meta_data.loc[row_idx, 'run']:
            ...
            try:
                ...
                log_cols = ('P_Strategy/P_Index/P_StartTime/P_EndTime/P_LastTradeTime/P_TradeInterval/P_OrderSide/P_Method/P_SL/P_OM/P_TillTime/P_ReEntries/Date/Day/DTE/Entry.Time/MMPS/')
                ...
                for current_date in date_lists:
                    ...
                    try:
                        ...
                        for idx, i in enumerate(range(0, parameter_len, chunk_size), start=1):
                            ...
                            chunk = [NRE_CC_PSL(bt, row.entry_time, row.exit_time, row.last_trade_time, row.trade_interval, row.orderside, row.method, row.sl, row.om, row.till_time, row.re_entries) for row in tqdm(chunk_parameter.itertuples(), total=len(chunk_parameter), colour='GREEN')]
